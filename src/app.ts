import {
  start as fakeRead,
  push as fakePush,
  contract as fakeContract
} from "./blockchains/fake"
...
let onRequest: RequestHandler = async req => {
  ...
  if (!def) return console.log(`data hash not found! ${req.dataHash}`), false

  let response = await providers[def.provider as keyof typeof providers](
    def.ident
  )
  let tx = await writers[req.blockchain as keyof typeof writers](
    req.receiver,
    req.dataHash,
    response
  )
...
apiConfig.generate = ({
  ...
  let e: IContractEndpointSettings = {
    name,
    type,
    lifetime: parseInt(lifetime),
    updateFreq: parseInt(updatefreq),
    hash: hashDataId({ category, provider, ident: slug })
  }
  let generator = generators[blockchain as keyof typeof generators]
  if (!generator) return { contract: "...", instructions: "..." }
